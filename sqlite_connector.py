...
class sqlite_db(object):
    ...
    def _delete_data(self, uuid=False):
        tables = ("audit", "policy_summary", "policy_details", "rules",
                  "endpoints", "alerts_facets_8", "alerts_facets_all",
                  "alert_severity_counts", "alerts_histogram", "users",
                  "tins", "sf_installation", "reports")
        for t in tables:
            cmd = f"delete from {t}"
            if uuid:
                cmd += f" where uuid = '{uuid}'"
            cmd += ";"
            self.execute(cmd)

    def _backup_table(self, table):
        with open(f"{table}_backup.sql", "w") as f:
            for line in self.connection.iterdump():
                if table in line:
                    print(line)
                    if f"create table {table}" in line.lower() or f'insert into "{table}"' in line.lower():
                        f.write(line)
        time.sleep(100)
        ...
        for chunk in chunks:
            ...
            for row in chunk:
                ...
                for x, i in enumerate(row):
                    ...
                    if not i and i != 0:
                        continue
                    elif isinstance(i, decimal.Decimal) or isinstance(i, int):
                        i = int(i)
